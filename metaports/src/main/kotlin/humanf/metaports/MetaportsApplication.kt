...
class MetaportsApplication

fun main(args: Array<String>) {
	runApplication<MetaportsApplication>(*args)
}
...
class BackgroundDataRestController(val BackgroundDataRepo: BackgroundDataRepository)
{
	...
	@DeleteMapping("BackgroundData")
	fun deleteAllBackgroundData(): ResponseEntity<Void> {
		BackgroundDataRepo.deleteAll()
...
class ProgramDataRestController(val ProgramDataRepo: ProgramDataRepository)
{
	...
	@DeleteMapping("ProgramData")
	fun deleteAllProgramData(): ResponseEntity<Void> {
		ProgramDataRepo.deleteAll()
...
class HandDataRestController(val HandDataRepo: HandDataRepository)
{
	...
	@PutMapping("HandData/{hand_id}")
	fun updateHandData(@PathVariable hand_id: Int, @RequestBody newHandData: hand): ResponseEntity<hand> {
		return HandDataRepo.findById(hand_id).map { existingHandData ->
			val updatedHandData: hand = existingHandData
					.copy(rx = newHandData.rx, ry = newHandData.ry, lx = newHandData.lx, ly = newHandData.ly)
			ResponseEntity.ok().body(HandDataRepo.save(updatedHandData))
